...
def upgrade():
    ...
    op.execute('CREATE TABLE offers (pokemon_id integer PRIMARY KEY REFERENCES pokemon(id), price_cents integer NOT NULL CHECK(price_cents > 0), stock integer NOT NULL CHECK(stock >= 0))')
    op.execute('CREATE TABLE api_cache (url text PRIMARY KEY, data jsonb NOT NULL, fetched_at timestamptz NOT NULL DEFAULT now())')
