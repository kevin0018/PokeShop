...
class CatalogTests(unittest.TestCase):
    ...
    def test_catalog_and_detail_agree(self):
        ...
        for item in result["items"]:
            self.assertEqual(
                self.client.get(f'/api/v1/pokemon/{item["id"]}').json(), item
            )
    ...
    def test_pagination_after_sorting(self):
        result = self.client.get(
            "/api/v1/pokemon?sort=price_desc&limit=1&offset=1"
        ).json()
        self.assertEqual(result["total"], 12)
    ...
    def test_empty_and_not_found(self):
        self.assertEqual(
            self.client.get("/api/v1/pokemon?q=missing").json(),
            {"items": [], "total": 0},
        )
    ...
    def test_invalid_query(self):
        for query in ("limit=101", "limit=0", "offset=-1", "sort=invalid"):
            self.assertEqual(
                self.client.get(f"/api/v1/pokemon?{query}").status_code, 422
            )
