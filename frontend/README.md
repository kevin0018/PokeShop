...
The homepage has a violet/yellow electric scene that appears immediately, without an opening overlay. Catalog cards follow compact six-item editorial blocks, independent of biological height; partial blocks preserve order. The cart paginates six products while keeping global totals, with a Chansey receipt scene. Statistics live inside the detail illustration. The animated Espeon/Umbreon switch initially follows the browser preference and remembers manual selections; page transitions remain 700 ms. Three-second pie countdowns pause on hover/focus. Reduced motion skips decorative animation. No database credentials belong in Vite variables.
